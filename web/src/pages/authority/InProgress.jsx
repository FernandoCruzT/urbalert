...
const BADGE = {
  asignado:   { bg: '#EFF6FF', color: '#2563EB' },
  en_proceso: { bg: '#FFF7ED', color: '#C2410C' },
};
...
export default function InProgress() {
  ...
  useEffect(() => {
    api.get('/reports/authority-reports', { params: { estado: 'activo' } })
  ...
  return (
    <AuthorityLayout>
      <div style={S.page}>
        <h2 style={S.title}>En proceso</h2>
        <div style={S.tableWrap}>
          {loading ? (
            <div style={S.empty}>Cargando…</div>
          ) : reportes.length === 0 ? (
            <div style={S.empty}>Sin reportes activos</div>
          ) : (
            <table style={S.table}>
              <thead>
                <tr>
              ...
              <tbody>
                {reportes.map(r => (
                  <tr
                    ...
                    onClick={() => navigate(`/authority/report/${r.id}`, { state: { from: '/authority/in-progress' } })}
                  >
