...
const S = {
  ...
  item:         (selected, leida) => ({
    padding: '0.75rem 0.9rem', borderBottom: '1px solid #F3F4F6', cursor: 'pointer',
  ...
  badge:        (leida) => ({ flexShrink: 0, display: 'inline-block', padding: '1px 7px', borderRadius: 20, fontSize: '0.68rem', fontWeight: 600, background: leida ? '#F3F4F6' : '#EFF6FF', color: leida ? '#6B7280' : 'var(--color-primary)' }),
...
function Detalle({ notif, onVerReporte }) {
  const ciudadano = notif.ciudadano_nombre ? `${notif.ciudadano_nombre} ${notif.ciudadano_apellido || ''}`.trim() : null;
  const autoridad = notif.autoridad_nombre ? `${notif.autoridad_nombre} ${notif.autoridad_apellido || ''}`.trim() : null;
  const hasMeta   = notif.reporte_id && (notif.categoria_nombre || notif.subcategoria_nombre || ciudadano || autoridad);
  ...
  return (
    <div style={S.detailScroll}>
      ...
      {hasMeta && (
        <>
          <div style={S.metaGrid}>
            {notif.categoria_nombre    && <div><div style={S.metaLabel}>Categoría</div><div style={S.metaValue}>{notif.categoria_nombre}</div></div>}
            {notif.subcategoria_nombre && <div><div style={S.metaLabel}>Subcategoría</div><div style={S.metaValue}>{notif.subcategoria_nombre}</div></div>}
            {ciudadano && <div><div style={S.metaLabel}>Ciudadano</div><div style={S.metaValue}>{ciudadano}</div></div>}
            {autoridad && <div><div style={S.metaLabel}>Autoridad</div><div style={S.metaValue}>{autoridad}</div></div>}
          </div>
          <hr style={S.divider} />
        </>
      )}
      ...
      {notif.reporte_id && (
        <button style={S.verBtn} onClick={() => onVerReporte(notif.reporte_id)}>Ver reporte</button>
      )}
    </div>
  );
}
    ...
    <AuthorityLayout>
      <div style={S.page}>
        ...
        <div style={S.body}>
          <div style={S.left}>
            <div style={S.listScroll}>
              {loading ? (
                ...
                notifs.map(n => (
                  <div key={n.id} style={S.item(selected?.id === n.id, n.leida)} onClick={() => handleSelect(n)}>
          ...
          <div style={S.right}>
            {selected
              ? <Detalle notif={selected} onVerReporte={id => navigate(`/authority/report/${id}`)} />
